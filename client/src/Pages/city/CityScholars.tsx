...
export default function CityScholars() {
  ...
  const filtered = scholars.filter((scholar) =>
    (scholar.name + ' ' + scholar.school + ' ' + scholar.barangay).toLowerCase().includes(query));
  ...
  return (
    <div>
      ...
      <div className="relative mb-5">
        ...
        <input value={search} onChange={(event) => setSearch(event.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[#E5E7EB] text-sm bg-white" placeholder="Search scholars by name, school or barangay..." />
      </div>
      <div className="bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E5E7EB] bg-[#F6F7F9]">
                {['Scholar', 'Course', 'School', 'Barangay', 'Email', 'Status'].map((header) => (
        ...
        {filtered.length === 0 && (
          <div className="p-10 text-center text-sm text-[#6B7280]">
            {scholars.length === 0
              ? 'No approved scholars yet. Scholars appear here once the City Office approves their existing-scholar claim on the Scholar Approval page.'
