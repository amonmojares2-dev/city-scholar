...
export default function SuperAdminScholars() {
  ...
  const filtered = scholars.filter((scholar) =>
    (scholar.name + ' ' + scholar.email + ' ' + scholar.school + ' ' + scholar.barangay).toLowerCase().includes(query));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input value={search} onChange={(event) => setSearch(event.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[#E5E7EB] text-sm bg-white" placeholder="Search by name, email, school or barangay..." />
      </div>
      {loading ? (
        ...
        <div className="overflow-auto rounded border">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {['Name', 'Email', 'School', 'Course / Year', 'Barangay', 'GWA', 'Verified'].map((header) => (
